...
namespace cc.newspring.AttendedCheckIn.Workflow.Action.CheckIn
{
    ...
    /// </summary>
    [Description( "Selects the grouptype, group, location and schedule for each person based on the group with the least number of people.  Ideal for a structure of groups with one-to-one relationships to locations." )]
    [Export( typeof( ActionComponent ) )]
    [ExportMetadata( "ComponentName", "Room Balance By Group" )]
        ...
        public override bool Execute( RockContext rockContext, Rock.Model.WorkflowAction action, Object entity, out List<string> errorMessages )
        {
            ...
            foreach ( var person in family.People.Where( p => p.Selected && p.GroupTypes.Any() ) )
            {
                ...
                if ( group != null && group.Locations.Any() )
                {
                    ...
                    if ( location == null )
                    {
                        // If no selected room pick the one with smallest current count
                        location = group.Locations.OrderBy( l => l.CurrentCount ).First();
                    }
